...
export function neighbors4Physical(gs, x, y) {
  const out = [];
  if (state.isOpenPhysical(gs, x, y, 'N')) out.push({ x,     y: y - 1 });
  if (state.isOpenPhysical(gs, x, y, 'E')) out.push({ x: x+1, y      });
  if (state.isOpenPhysical(gs, x, y, 'S')) out.push({ x,     y: y + 1 });
  if (state.isOpenPhysical(gs, x, y, 'W')) out.push({ x: x-1, y      });
  return out;
}
...
// Physical BFS (editor/connectivity checks only)
export function floodFromPhysical(gs, starts) {
  const startList = Array.isArray(starts) ? starts
                    : (starts && typeof starts === 'object') ? [starts]
                    : [ state.EXIT ];
  const w = state.GRID.cols, h = state.GRID.rows;
  ...
  while (head < q.length) {
    const { x, y } = q[head++];
    const d = dist[y][x];
    for (const n of neighbors4Physical(gs, x, y)) {
      if (dist[n.y][n.x] > d + 1) {
        dist[n.y][n.x] = d + 1;
        q.push(n);
      }
...
// Connectivity check used by the editor: test physical graph only
export function isEntryConnectedToExit(gs, entry = state.ENTRY, exit = state.EXIT) {
  const D = floodFromPhysical(gs, exit);
